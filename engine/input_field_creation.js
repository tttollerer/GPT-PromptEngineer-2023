// Global UI creation functions for PromptEngineer Extension
// These functions are immediately available when this script loads
...
window.createDropdown = function createDropdown(dropdownData) {
  ...
  const dropdownElement = window.createInputField('select', { options: dropdownData.options });
...
window.createInput = function createInput(inputData) {
  ...
  const inputElement = window.createInputField('input', { type: 'text', id: inputData.id });
...
// Optional: Legacy function for backwards compatibility
